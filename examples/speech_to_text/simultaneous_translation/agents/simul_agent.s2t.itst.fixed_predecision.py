...
class OnlineFeatureExtractor:
    ...
    def __call__(self, new_samples):
        ...
        num_frames = math.floor(
            (len(samples) - self.len_ms_to_samples(self.window_size - self.shift_size)) / self.num_samples_per_shift
        )
...
class FairseqSimulSTAgent(SpeechAgent):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--test-threshold", type=float, default=0.2,
    ...
    def policy(self, states):
        ...
        tgt_indices = self.to_device(
            torch.LongTensor(
                [self.model.decoder.dictionary.eos()] + [x for x in states.units.target.value if x is not None]
            ).unsqueeze(0)
        )
        ...
        # logger.info("outputs: {}".format(outputs.keys()))
