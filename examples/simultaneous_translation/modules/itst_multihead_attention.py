...
class InformationTransportAttention(MultiheadAttention):
    ...
    def monotonic_attention_process_infer(
        ...
        alpha = _p_choose.new_zeros([self.num_heads, src_len]).scatter(
            1,
            (monotonic_step.unsqueeze(0)).repeat(self.num_heads, 1).clamp(0, src_len - 1),
            1,
        )
    ...
    def monotonic_attention_process_train(
        ...
        # 1. compute stepwise probability
        p_choose, p_choose_copy, p_choose_pooled, key_padding_mask_pool = self.p_choose(query, key, key_padding_mask)
    ...
    def forward(
        ...
        if step is not None:
            ...
            (
                ...
                soft_energy,
                key_padding_mask_pool,
            ) = self.monotonic_attention_process_train(query, key, key_padding_mask, train_threshold)
