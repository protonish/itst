...
def rw_with_info(
    ...
    if train_threshold is not None:
        max_id = (p_choose.cumsum(dim=-1) < train_threshold).sum(dim=-1, keepdim=True) + 1
        max_id = torch.cummax(max_id, dim=-2)[0]
        max_id = max_id.max(
            (torch.arange(1, tgt_len + 1, device=p_choose.device) * pre_decision_ratio).unsqueeze(0).unsqueeze(2)
        ).clamp(1, src_len)
        max_id = (max_id + pre_decision_ratio).clamp(1, src_len)
        info_mask = (
            torch.arange(1, src_len + 1, device=max_id.device).unsqueeze(0).unsqueeze(0).repeat(bsz, tgt_len, 1)
        )
        alpha = (info_mask == max_id).type_as(p_choose)

    if test_threshold is not None:
        max_id = (p_choose.cumsum(dim=-1) < test_threshold).sum(dim=-1, keepdim=True) + 1
        max_id = torch.cummax(max_id, dim=-2)[0]
        max_id = max_id.max(
            (torch.arange(1, tgt_len + 1, device=p_choose.device) * pre_decision_ratio).unsqueeze(0).unsqueeze(2)
        )
